...
export function normalizeRoles(roles?: UserRoles | number | string | null, legacyRole?: number | string): UserRoles {
  ...
  if (isRolesObject(roles)) {
    ...
    const legacyKey = roleKeyForCode(legacyCode)
    normalized[legacyKey] = legacyCode
  }
...
export function hasRole(roles: UserRoles | number | string | null | undefined, role: UserRole, legacyRole?: number | string) {
  return Object.values(normalizeRoles(roles, legacyRole)).includes(role)
}

export function isSuperAdminRole(roles?: UserRoles | number | string | null, legacyRole?: number | string) {
  return hasRole(roles, USER_ROLES.SUPER_ADMIN, legacyRole)
}

export function canReviewKyc(roles?: UserRoles | number | string | null, legacyRole?: number | string) {
  return hasRole(roles, USER_ROLES.KYC_REVIEWER, legacyRole) || isSuperAdminRole(roles, legacyRole)
}

export function canResolveDisputes(roles?: UserRoles | number | string | null, legacyRole?: number | string) {
  return hasRole(roles, USER_ROLES.DISPUTE_RESOLVER, legacyRole) || isSuperAdminRole(roles, legacyRole)
}
...
export function adminHomeForRoles(roles?: UserRoles | number | string | null, legacyRole?: number | string) {
  if (hasRole(roles, USER_ROLES.KYC_REVIEWER, legacyRole) && !isSuperAdminRole(roles, legacyRole)) return '/admin/kyc'
  if (hasRole(roles, USER_ROLES.DISPUTE_RESOLVER, legacyRole) && !isSuperAdminRole(roles, legacyRole)) return '/admin/disputes'
  return '/admin'
}
